"use client";
import React, { useContext, useState } from "react";
import { useForm } from "react-hook-form";
import { useRouter } from "next/navigation";
import Image from "next/image";
import Link from "next/link";
import { AuthContext } from "../contexts/UserProvider";

const RegisterPage = () => {
  const {
    register,
    formState: { errors },
    handleSubmit,
  } = useForm();

  const [loginError, setLoginError] = useState("");
  const { registerUser } = useContext(AuthContext);
  const router = useRouter();
  const onSubmitRegister = async (data) => {
    const { name, email, phone, password, refferCode, photos } = data;

    await registerUser(name, email, phone, password, refferCode, photos);
    router.push("/");
  };

  const [showPassword, setShowPassword] = useState(false);
  const [password, setPassword] = useState("");

  const toggleShowPassword = () => {
    setShowPassword(!showPassword);
  };

  const handlePasswordChange = (event) => {
    setPassword(event.target.value);
  };
  return (
    <div>
      <div className="container myaccount">
        <div className="col-lg-10 col-sm-12 mx-auto login-div">
          <div className="row">
            <div className="col-lg-6 col-md-12 col-sm-12 mx-auto bg-dark p-0">
              <Image
                src="https://plus.unsplash.com/premium_photo-1675799807356-6e97c49f22ce?q=80&w=1964&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                width={700}
                height={200}
              />
            </div>
            <div className="col-lg-6 col-md-12 col-sm-12">
              <div className="col-sm-10 mx-auto py-4 my-4">
                <h1>Create a new account</h1>
                <form onSubmit={handleSubmit(onSubmitRegister)}>
                  <input
                    type="hidden"
                    name="_token"
                    defaultValue="KgvbWDJZDDwud524LbE5WM5ynJwTQuhfyLSb8unR"
                  />{" "}
                  <div className="form-group row">
                    <div className="col-md-12 mb-3">
                      <label className="d-block">Name</label>
                      <input
                        id="name"
                        type="text"
                        className="form-control p-3 "
                        name="name"
                        required
                        placeholder="Name"
                        {...register("name", {
                          required: "Name is Required",
                        })}
                      />
                      {errors.firstName && (
                        <p className="text-red-500">{errors.name.message}</p>
                      )}
                    </div>
                    <div className="col-md-12 mb-3">
                      <label className="d-block">Email</label>
                      <input
                        id="email"
                        type="email"
                        className="form-control p-3 "
                        name="email"
                        required
                        placeholder="Email"
                        {...register("email", {
                          required: true,
                        })}
                      />
                      {errors.email && (
                        <p className="text-red-500">{errors.email.message}</p>
                      )}
                    </div>
                    <div className="col-md-12 mb-3">
                      <label className="d-block">Number</label>
                      <input
                        id="tesxt"
                        type="text"
                        className="form-control p-3 "
                        name="phone"
                        required
                        placeholder="Phone Number"
                        {...register("phone", {
                          required: true,
                        })}
                      />
                      {errors.phone && (
                        <p className="text-red-500">{errors.phone.message}</p>
                      )}
                    </div>

                    <div className="col-md-12 mb-3">
                      <label className="d-block">Password</label>
                      <input
                        id="password"
                        type="password"
                        className="form-control p-3 "
                        name="password"
                        required
                        placeholder="Password"
                        {...register("password", {
                          required: "Password is required",
                          minLength: {
                            value: 6,
                            message: "Password must be 6 characters long",
                          },
                        })}
                      />
                    </div>
                  </div>
                  <div className="form-group mx-auto">
                    <div
                      className="g-recaptcha"
                      data-sitekey="6Lcjb78UAAAAAIl2fokM1Ap0XAI4QfJ9WOyCE1Ql"
                    />
                  </div>
                  <div className="form-group">
                    <button
                      type="submit"
                      className="btn btn-site btn-block py-2"
                    >
                      <i className="fa fa-key mr-1" /> Register{" "}
                    </button>
                  </div>
                </form>
                <p className="text-center my-3">OR</p>
                <Link
                  className="btn btn-block border border-dark text-dark btn-link bg-white py-2 mb-2 text-center"
                  href="/login"
                >
                  Already Registered? Login Here
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default RegisterPage;
